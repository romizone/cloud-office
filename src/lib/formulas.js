...
export function displayOf(val, fmt, opts = {}) {
  ...
  if (val.t === 'n') {
    ...
    const serialDate = () => new Date(Math.round((val.v - 25569) * 86400000))
    if (fmt === 'date') return serialDate().toLocaleDateString('id-ID', { timeZone: 'UTC' })
    if (fmt === 'longdate') return serialDate().toLocaleDateString('id-ID', { dateStyle: 'long', timeZone: 'UTC' })
    if (fmt === 'time') return serialDate().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', timeZone: 'UTC' })
